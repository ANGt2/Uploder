...
async def get_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تبدیل پسورد به فرمت هش‌شده rclone
    obs = subprocess.run(["rclone", "obscure", acc_pass], capture_output=True, text=True)
    obscured_pass = obs.stdout.strip() if obs.returncode == 0 and obs.stdout.strip() else acc_pass

    # ساخت قطعی کانفیگ
    subprocess.run(
        ["rclone", "config", "create", remote_name, srv_type, f"user={acc_user}", f"pass={obscured_pass}"],
        capture_output=True, text=True
    )
...
if __name__ == '__main__':
    ...
    print("🚀 ربات با چیدمان و ساختار کامل اولیه اجرا شد...")
    app.run_polling()
