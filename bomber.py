...
def send_requests(phone_number):
    headers = {
        "Content-Type": "application/json",
        "User-Agent": get_random_user_agent()  # Random User-Agent
    }

    # List of APIs to send requests to
    apis = [
        {
            "name": "Snapp V1",
    ...
    # Send requests to all APIs
    for api in apis:
        success, message = send_request(api["url"], api["data"], headers)
        print(f"{api['name']}: {message}")

# Main function
def main():
    print("Welcome to the API Request Sender!")
    phone_number = input("Please enter your phone number (e.g., 09123456789): ")
