...
def init_db():
    ...
    default_products = [
        ("UV", "Uvoucher", "UV", 1, 1, 0, 8),
        ("HV", "Hot Voucher", "HV", 1, 1, 0, 8),
        ("PV", "Premium Voucher", "PV", 1, 1, 0, 8),
    ]

    for code, name, prefix, active, min_amount, max_amount, profit in default_products:
        cur.execute("""
        INSERT OR IGNORE INTO products
        (code, name, prefix, is_active, min_amount, max_amount, profit_percent, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (code, name, prefix, active, min_amount, max_amount, profit, now(), now()))
    ...
    for col, ddl in {
        ...
        "tracking_code": "ALTER TABLE orders ADD COLUMN tracking_code TEXT",
        "product_type": "ALTER TABLE orders ADD COLUMN product_type TEXT DEFAULT 'UV'",
...
def get_product_rate(product_code="UV"):
    try:
        req = urllib.request.Request(
            NOBITEX_USDT_URL,
            headers={"User-Agent": "MixVoucherBot"}
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT profit_percent FROM products WHERE code=?", (product_code,))
    row = cur.fetchone()
    con.close()

    profit = float(row[0]) if row else float(get_setting("profit_percent", DEFAULT_PROFIT_PERCENT))
    return int(usdt * (1 + profit / 100))


def get_product_row(code):
    con = db()
    cur = con.cursor()
    cur.execute("""
    SELECT code, name, prefix, is_active, min_amount, max_amount, profit_percent
    FROM products
    WHERE code=?
    """, (code,))
    row = cur.fetchone()
...
def active_products_keyboard():
    ...
    for code, name in rows:
        buttons.append([
            InlineKeyboardButton(f"🎫 {name}", callback_data=f"buy_product_{code}")
        ])
...
def product_status_icon(active):
    return "✅ فعال" if int(active) == 1 else "❌ غیرفعال"
...
def admin_products_keyboard():
    ...
    buttons = []
    for code, name, prefix, active, min_amount, max_amount, profit in rows:
        buttons.append([
            InlineKeyboardButton(
                f"{product_status_icon(active)} | {code} | {name}",
                callback_data=f"admin_product_{code}"
            )
        ])
...
def admin_product_text(row):
    code, name, prefix, active, min_amount, max_amount, profit = row
    max_text = "نامحدود" if float(max_amount or 0) == 0 else f"{max_amount}$"

    return (
        "🛍 جزئیات محصول\n\n"
        f"کد محصول: {code}\n"
        f"نام: {name}\n"
        f"پیشوند سفارش: {prefix}\n"
        f"وضعیت: {product_status_icon(active)}\n"
        f"حداقل سفارش: {min_amount}$\n"
        f"حداکثر سفارش: {max_text}\n"
        f"درصد سود: {profit}%"
    )


def admin_product_keyboard(code, active):
    toggle_text = "❌ غیرفعال کردن" if int(active) == 1 else "✅ فعال کردن"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(toggle_text, callback_data=f"admin_product_toggle_{code}")],
        [InlineKeyboardButton("🔙 مدیریت محصولات", callback_data="admin_products")]
    ])
...
def order_detail_text(order_id, user_id):
    ...
    SELECT id, amount_usd, price_toman, status, created_at, updated_at,
           order_number, tracking_code, product_type, completed_at, voucher_code, reject_reason
    FROM orders
    ...
    oid, amount, price, status, created_at, updated_at, order_number, tracking_code, product_type, completed_at, voucher_code, reject_reason = o

    delivered_at = completed_at or "—"
    voucher = voucher_code if voucher_code else "هنوز ارسال نشده است."
    reject_line = f"\n\n📝 دلیل رد\n{reject_reason}" if status == "rejected" and reject_reason else ""
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID:
        ...
        if admin_mode == "add_product_code":
            code = text.strip().upper().replace(" ", "")

            if not code.isalnum() or len(code) < 2 or len(code) > 6:
                await update.message.reply_text("کد محصول نامعتبر است. مثال: GV یا UV یا CHAT")
                return
            ...
            if exists:
                await update.message.reply_text("این کد محصول از قبل وجود دارد. یک کد دیگر بفرست.")
                return

            context.user_data["new_product_code"] = code
            context.user_data["admin_mode"] = "add_product_name"
            await update.message.reply_text("نام محصول را بفرست. مثال: Gift Voucher")
            return

        if admin_mode == "add_product_name":
            name = text.strip()

            if len(name) < 2:
                await update.message.reply_text("نام محصول خیلی کوتاه است.")
                return

            context.user_data["new_product_name"] = name
            context.user_data["admin_mode"] = "add_product_prefix"
            await update.message.reply_text("پیشوند شماره سفارش را بفرست. مثال: GV")
            return

        if admin_mode == "add_product_prefix":
            prefix = text.strip().upper().replace(" ", "")
            code = context.user_data.get("new_product_code")
            name = context.user_data.get("new_product_name")

            if not code or not name:
                context.user_data.clear()
                await update.message.reply_text("اطلاعات محصول ناقص شد. دوباره از پنل محصولات شروع کن.")
                return

            if not prefix.isalnum() or len(prefix) < 2 or len(prefix) > 6:
                await update.message.reply_text("پیشوند نامعتبر است. مثال: GV یا UV")
                return
    ...
    if text == "🛒 خرید ووچر":
        ...
        if count == 0:
            await update.message.reply_text("فعلاً هیچ محصول فعالی برای خرید وجود ندارد.")
            return

        await update.message.reply_text(
            "🛒 خرید ووچر\n\nمحصول موردنظر را انتخاب کنید:",
            reply_markup=active_products_keyboard()
        )
        return
    ...
    if mode == "buy_amount":
        ...
        product_code = context.user_data.get("product_code", "UV")
        product = get_product_row(product_code)

        if not product or int(product[3]) != 1:
            context.user_data.clear()
            await update.message.reply_text("این محصول در حال حاضر فعال نیست یا پیدا نشد.", reply_markup=main_menu())
            return

        _, product_title, product_prefix, _, min_amount, max_amount, _ = product

        if amount < float(min_amount or 0):
            await update.message.reply_text(f"حداقل سفارش برای {product_title}: {min_amount}$")
            return

        if float(max_amount or 0) > 0 and amount > float(max_amount):
            await update.message.reply_text(f"حداکثر سفارش برای {product_title}: {max_amount}$")
            return

        rate = get_product_rate(product_code)
        ...
        cur = con.cursor()
        cur.execute("""
        INSERT INTO orders(user_id, amount_usd, rate, price_toman, status, product_type, created_at, updated_at)
        VALUES(?,?,?,?,?,?,?,?)
        """, (user.id, amount, rate, price, "waiting_receipt", product_code, now(), now()))
        order_id = cur.lastrowid
        con.commit()
        con.close()

        try:
            identity = ensure_order_identity(order_id, product_code)
            order_number, tracking_code = identity if identity else (f"#{order_id}", "-")
        except Exception as e:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("buy_product_"):
        code = data.replace("buy_product_", "")
        product = get_product_row(code)

        if not product or int(product[3]) != 1:
            await query.message.reply_text("این محصول فعال نیست یا پیدا نشد.", reply_markup=main_menu())
            return
        ...
        _, name, prefix, active, min_amount, max_amount, profit = product
        max_text = "نامحدود" if float(max_amount or 0) == 0 else f"{max_amount}$"

        await query.message.reply_text(
            f"🎫 محصول انتخابی: {name}\n\n"
            f"حداقل سفارش: {min_amount}$\n"
            f"حداکثر سفارش: {max_text}\n\n"
            "لطفاً مبلغ دلاری موردنظر را وارد کنید.\n\n"
            "مثال: 10",
            reply_markup=buy_start_keyboard()
        )
        return
    ...
    if data == "admin_products":
        ...
        if count == 0:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 پنل ادمین", callback_data="admin_home")]])
            )
            return
    ...
    if data == "admin_product_add":
        context.user_data.clear()
        context.user_data["admin_mode"] = "add_product_code"
        await query.message.reply_text(
            "➕ افزودن محصول جدید\n\n"
            "کد محصول را بفرست.\n"
            "مثال: GV یا CHAT یا AI"
        )
        return

    if data.startswith("admin_product_toggle_"):
        code = data.replace("admin_product_toggle_", "")

        con = db()
        cur = con.cursor()
        cur.execute("SELECT is_active FROM products WHERE code=?", (code,))
        row = cur.fetchone()
        ...
        await query.edit_message_text(
            ...
            reply_markup=admin_product_keyboard(code, product[3])
        )
        return

    if (
        data.startswith("admin_product_")
        and not data.startswith("admin_product_toggle_")
    ):
        code = data.replace("admin_product_", "")
